			...
			<div class="col-lg-7">
				<nav class="breadcrumb-container" aria-label="breadcrumb">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item">
							<a href="{{ route('admin.farmer', [$farmer['id']]) }}">{{ $farmer['name'] }}</a>
						</li>
		...
		<div class="col-md-12">
			<div class="card table-card">
				<div class="card-header">
					<form class="forms-sample ajax-get-report" id="view_farmer_report_form" action="{{ route('farmer.report', $farmer['id']) }}" method="post">
					@csrf
					...
					<div class="d-inline-block mr-2 mb-3 mb-lg-0">
						<label>&nbsp;</label>
						<button type="submit" id="view_farmer_report_form_submit" style="padding: 8px 15px 8.5px 15px; border-radius: 4px;" class="border-0 bg-success text-white">
						Submit
						</button>
					</div>
					</form>
				</div>
				<div class="card-block" id="view_farmer_report_form_feedback">
					<x-account.farm.farmer_report_table :seasons=$seasons :from=$from :to=$to />
				</div>
			</div>
		</div>
	</div>
